...
use Carbon\Carbon;
use Etlok\Crux\WebSockets\Cache\RedisLock;
use Etlok\Crux\WebSockets\Server\MockableConnection;
...
class RedisChannelManager extends LocalChannelManager
{
    ...
    public function __construct(LoopInterface $loop, $factoryClass = null)
    {
        ...
        $this->redis = Redis::connection(
            config('crux_websockets.replication.modes.redis.connection', 'default')
        );
    ...
    public function getChannelMembers($appId, string $channel): PromiseInterface
    {
        return $this->publishClient
            ...
            ->then(function ($list) {
                return collect($this->redisListToArray($list))->map(function ($user) {
     ...
     * @return array
     */
    public function redisListToArray(array $list)
    {
    ...
    {
        $name = config('crux_websockets.replication.modes.redis.connection', 'default');
